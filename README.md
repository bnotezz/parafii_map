# Візуалізація Каталогу метричних книг ДАРО
...
Для автоматичного розгортання Cloudflare Worker (триггер на коміт) можна скористатися зручною npm-ціллю:

```sh
npm run deploy:worker
```

Це виконає `wrangler deploy` у директорії `workers/daro-parser`, де знаходиться конфігурація для воркера. Якщо ваш CI виконує команди з кореня репозиторію, використовуйте саме цю ціль.
